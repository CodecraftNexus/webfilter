...
exports.getAllWebFilters = exports.deleteWebFilter = exports.updateWebFilter = exports.createWebFilter = exports.getWebFilterById = exports.getWebFilters = void 0;
...
exports.getAllWebFilters = (0, asyncHandler_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const userId = req.user.userId;
    const { list_type, type, from, to, page = "1", limit = "20", } = req.query;
    const where = { user_id: userId };
    if (list_type && ["white", "black"].includes(list_type)) {
        where.list_type = list_type;
    }
    ...
    const limitNum = Math.min(100, Math.max(1, parseInt(limit)));
    const offset = (pageNum - 1) * limitNum;
    const [applicationResults, linkResults] = yield Promise.all([
        db_1.db.WebFilter.findAndCountAll({
            where,
            order: [["datetime", "DESC"]],
            limit: limitNum,
            offset,
        }),
        db_1.db.WebFilterLink.findAndCountAll({
            where,
            order: [["datetime", "DESC"]],
            limit: limitNum,
            offset,
    ...
    const applications = applicationResults.rows.map((r) => (Object.assign(Object.assign({}, r.toJSON()), { source: "application" })));
    const links = linkResults.rows.map((r) => (Object.assign(Object.assign({}, r.toJSON()), { source: "link" })));
    return res.json({
        success: true,
        data: {
            applications: {
                records: applications,
                total: applicationResults.count,
            },
            links: {
                records: links,
